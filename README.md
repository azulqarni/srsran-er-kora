...
For the RuSIM+CoreSIM setup, run the monolithic gNB with:

```bash
~/srsRAN_ER/build/apps/gnb/gnb -c ~/configs/srsgnb_4x4_100mhz_slicing.yaml
```

For the RuSIM+CuSIM setup (where CuSIM includes core emulation), run the DU with:

```bash
~/srsRAN_ER/build/apps/du/srsdu -c ~/configs/srsdu_4x4_100mhz.yaml
```
...
This repository extends [EdgeRIC-on-5G](https://github.com/ucsdwcsng/EdgeRIC-on-5G), where dependencies are specified. The gNB exchanges runtime metrics and controls with EdgeRIC over ZeroMQ PUB/SUB sockets, and both telemetry messages include a TTI index:

- Per-UE telemetry (`ipc:///tmp/metrics`, message `Metrics`): published at the end of each TTI. Per UE it carries `rnti`, `cqi`, `snr`, `tx_bytes`, `rx_bytes`, `dl_buffer`, `ul_buffer`, `dl_tbs`, plus `tti_cnt` for the slot index.
- Per-slice telemetry (`ipc:///tmp/slice_metrics`, message `SliceMetrics`): published each TTI after intra-slice scheduling. Per slice it carries `slice_id`, current `cfg_min_prb`, `cfg_max_prb`, `cfg_priority`, `dl_rbs_used`, `ul_rbs_used`, `avg_dl_rbs_per_slot`, `avg_ul_rbs_per_slot`, and `hol_slots` (max DL HoL age in slots).
- Per-UE control (`ipc:///tmp/control_weights_actions`, `ipc:///tmp/control_mcs_actions`): non-blocking SUBs read scheduling weights and MCS overrides keyed by RNTI at the start of each slot.
- Per-slice control (`ipc:///tmp/control_slice_budgets`): non-blocking SUB reads `SliceBudgets` (min_prb, max_prb, priority per slice) at the start of inter-slice `slot_indication`.

Proto definitions live under `lib/protobufs/`; generated C++ stubs are in `lib/edgeric/`; generated Python stubs are in `edgeric-v2/protobufs/` (older local variants are archived in `edgeric-v2/protobufs/original/`).
