...
def eval_loop_weight(eval_episodes, idx_algo):
    ...
    for i_episode in range(eval_episodes):
        ...
        # algo5 Single UE throttle
        if(idx_algo == 5):
            weights = algo5_single_ue_throttle()
            if weights.size == 0:
                continue
            edgeric_messenger.send_scheduling_weight(edgeric_messenger.ran_tti, weights, False)
            value_algo = "Single UE Priority"
...
def algo5_single_ue_throttle():
    ...
    priority_rnti_raw = None
    try:
        priority_rnti_raw = redis_db.get('priority_ue_rnti')
    except NameError:
        priority_rnti_raw = None

    priority_index = 0
    if priority_rnti_raw:
        priority_index = next((idx for idx, rnti in enumerate(RNTIs) if str(rnti) == priority_rnti_raw), None)
        if priority_index is None:
            try:
                priority_rnti = int(priority_rnti_raw, 0)
                priority_index = next((idx for idx, rnti in enumerate(RNTIs) if rnti == priority_rnti), None)
            except ValueError:
                priority_index = None
        if priority_index is None:
            priority_index = 0

    throttled_total_share = 0.05 if numues > 1 else 0.0
    boosted_share = 1.0 - throttled_total_share
    throttled_share = throttled_total_share / (numues - 1) if numues > 1 else 0.0

    for i in range(numues):
        weights[i*2] = RNTIs[i]
        weights[i*2 + 1] = boosted_share if i == priority_index else throttled_share
