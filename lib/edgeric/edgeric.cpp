...
std::map<uint16_t, uint64_t> edgeric::rx_bytes = {};
std::map<uint16_t, uint64_t> edgeric::tx_bytes = {};
...
std::map<uint16_t, uint32_t> edgeric::ue_slice_ids = {};
...
zmq::socket_t slice_publisher(context, ZMQ_PUB);
zmq::socket_t subscriber_weights(context, ZMQ_SUB);
zmq::socket_t subscriber_mcs(context, ZMQ_SUB);
zmq::socket_t subscriber_slice_budgets(context, ZMQ_SUB);

static std::map<uint32_t, edgeric::slice_budget> slice_budgets_recved;

void edgeric::init() {
    publisher.bind("ipc:///tmp/metrics");
    slice_publisher.bind("ipc:///tmp/slice_metrics");
    ...
    int conflate = 1;
    subscriber_weights.set(zmq::sockopt::conflate, conflate);
    ...
    subscriber_mcs.set(zmq::sockopt::conflate, conflate);

    subscriber_slice_budgets.connect("ipc:///tmp/control_slice_budgets");
    subscriber_slice_budgets.set(zmq::sockopt::subscribe, "");
    subscriber_slice_budgets.set(zmq::sockopt::conflate, conflate);
...
void edgeric::send_to_er() {
    ...
    for (const auto& ue_cqi_pair : ue_cqis) {
        ...
        ue_metrics->set_tx_bytes((tx_bytes_it != tx_bytes.end())
                                     ? static_cast<float>(tx_bytes_it->second)
                                     : 0.0f);

        // Set Rx Bytes, default to 0 if not available
        auto rx_bytes_it = rx_bytes.find(rnti);
        ue_metrics->set_rx_bytes((rx_bytes_it != rx_bytes.end())
                                     ? static_cast<float>(rx_bytes_it->second)
                                     : 0.0f);
        ...
        // Set slice ID, default to 0 if not available
        auto slice_it = ue_slice_ids.find(rnti);
        ue_metrics->set_slice_id(slice_it != ue_slice_ids.end() ? slice_it->second : 0);
    }
...
void edgeric::send_slice_metrics(uint32_t tti, const std::vector<slice_metric>& metrics)
{
    ensure_initialized();
    ...
    for (const auto& m : metrics) {
        ...
        slice->set_slice_id(m.slice_id);
        slice->set_cfg_min_prb(m.cfg_min_prb);
        slice->set_cfg_max_prb(m.cfg_max_prb);
        slice->set_cfg_priority(m.cfg_priority);
        slice->set_dl_rbs_used(m.dl_rbs_used);
        slice->set_ul_rbs_used(m.ul_rbs_used);
        slice->set_avg_dl_rbs_per_slot(m.avg_dl_rbs_per_slot);
        slice->set_avg_ul_rbs_per_slot(m.avg_ul_rbs_per_slot);
        slice->set_hol_slots(m.hol_slots);
    }

    std::string serialized_msg;
    if (!msg.SerializeToString(&serialized_msg)) {
        std::cerr << "Failed to serialize SliceMetrics message." << std::endl;
        return;
    }

    zmq::message_t zmq_msg(serialized_msg.size());
    memcpy(zmq_msg.data(), serialized_msg.data(), serialized_msg.size());
    slice_publisher.send(zmq_msg, zmq::send_flags::dontwait);
}

std::map<uint32_t, edgeric::slice_budget> edgeric::get_slice_budgets()
{
    ensure_initialized();

    zmq::message_t recv_message;
    zmq::recv_result_t size = subscriber_slice_budgets.recv(recv_message, zmq::recv_flags::dontwait);

    if (size) {
        SliceBudgets budgets_msg;
        if (budgets_msg.ParseFromArray(recv_message.data(), recv_message.size())) {
            slice_budgets_recved.clear();
            for (const auto& b : budgets_msg.budgets()) {
                slice_budget sb{b.min_prb(), b.max_prb(), b.priority()};
                slice_budgets_recved[b.slice_id()] = sb;
            }
        } else {
            std::cerr << "Failed to parse SliceBudgets message." << std::endl;
        }
    }

    return slice_budgets_recved;
}
...
void edgeric::get_weights_from_er()
{
    ...
    if (size) {
        ...
        // Keep previously received weights so the gNB continues to apply the latest policy
        // until a new update arrives. Clearing here causes most TTIs to see no weights.
    }
}
