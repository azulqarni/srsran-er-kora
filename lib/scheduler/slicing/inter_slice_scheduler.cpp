...
{
  // Apply dynamic slice budgets if provided.
  const auto budgets = edgeric::get_slice_budgets();
  if (not budgets.empty()) {
    const unsigned cell_max_rbs = cell_cfg.dl_cfg_common.init_dl_bwp.generic_params.crbs.length();
    for (auto& s : slices) {
      auto it = budgets.find(s.inst.id.value());
      if (it == budgets.end()) {
        continue;
      }
      const auto& b = it->second;
      s.inst.cfg.min_prb = std::min(b.min_prb, cell_max_rbs);
      s.inst.cfg.max_prb = std::min(std::max(b.max_prb, s.inst.cfg.min_prb), cell_max_rbs);
      s.inst.cfg.priority = std::min(b.priority, slice_rrm_policy_config::max_priority);
    }
  }
...
std::vector<edgeric::slice_metric> inter_slice_scheduler::collect_slice_metrics() const
{
  std::vector<edgeric::slice_metric> metrics;
  metrics.reserve(slices.size());

  for (const auto& s : slices) {
    unsigned max_hol_slots = 0;
    if (current_slot.valid()) {
      for (const slice_ue& u : s.inst.get_ues()) {
        auto lcs = u.logical_channels();
        if (not lcs.has_value()) {
          continue;
        }
        for (const auto& lc_cfg : *lcs) {
          const lcid_t lcid = lc_cfg->lcid;
          slot_point    hol_toa = u.dl_hol_toa(lcid);
          if (hol_toa.valid() and current_slot >= hol_toa) {
            unsigned hol_slots = current_slot - hol_toa;
            max_hol_slots      = std::max(max_hol_slots, hol_slots);
          }
        }
      }
    }

    edgeric::slice_metric m{};
    m.slice_id             = s.inst.id.value();
    m.cfg_min_prb          = s.inst.cfg.min_prb;
    m.cfg_max_prb          = s.inst.cfg.max_prb;
    m.cfg_priority         = s.inst.cfg.priority;
    m.dl_rbs_used          = s.inst.pdsch_rb_count;
    m.ul_rbs_used          = std::accumulate(s.inst.pusch_rb_count_per_slot.begin(),
                                             s.inst.pusch_rb_count_per_slot.end(),
                                             0U);
    m.avg_dl_rbs_per_slot  = s.inst.average_pdsch_rbs_per_slot();
    m.avg_ul_rbs_per_slot  = s.inst.average_pusch_rbs_per_slot();
    m.hol_slots            = max_hol_slots;
    metrics.push_back(m);
  }
